...
/**
 * 
 * @param {object} event Triggered by the user either clicking the enter button 
 * or by using the mouse and clicking on the search icon
 */

function checkIfEnter(event) {
  if (event.keyCode === 13 || event.type === 'click') {
    if (!enquiryRef.value) return;
 ...
 * If the location that the user searches for is invalid, 
 * this function will catch the error
 * 
 * @param {string} city The location the user is searching 
 * to find out the weather conditions
 * 
...
/**
 * @param {any} () clears the weather data on the screen if there is an error
 */
...
/**
 * displays area name, date, temperature, conditions and min and max temperature
 * 
 * @param {any} weather displays the weather conditions 
